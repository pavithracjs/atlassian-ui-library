...
- to run all tests under certain directories on local `yarn test:vr <relative_path_to_directory>`.
- to run a single test on local `yarn test:vr <absolute_path_to_file>`.
- to run a single test on local using *watch mode* :
    ...
    - `yarn test:vr <absolute_path_to_file> --watch` will run watch mode headlessly.
    - `yarn test:vr <absolute_path_to_file> --debug` will run watch mode only on Chrome browser.
- to update all image snapshot for the package `yarn test:vr <pkg> --update-snapshot` or `yarn test:vr <pkg> -u`
- to update image snapshot for a single test `yarn test:vr <absolute_path_to_file> --update-snapshot` or `yarn test:vr <absolute_path_to_file> -u` will update the snapshot if there is a change.  
**Note:** you can still use the `--watch` and `--debug` flags with `<pkg>`, `<relative_path_to_directory>` and for all tests but it is not recommended.
