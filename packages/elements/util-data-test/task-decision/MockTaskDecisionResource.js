...
var MockTaskDecisionResource = /** @class */ (function() {
  ...
  MockTaskDecisionResource.prototype.getDecisions = function(query) {
    ...
    var result = convertServiceDecisionResponseToDecisionResponse(
  ...
  MockTaskDecisionResource.prototype.unsubscribeRecentUpdates = function(
    _id,
  ) {};
  MockTaskDecisionResource.prototype.notifyRecentUpdates = function(
    _updateContext,
  ) {};
  ...
  MockTaskDecisionResource.prototype.applyConfig = function(
    query,
    result,
    itemKey,
  ) {
    ...
    if (this.config && this.config.hasMore) {
      nextQuery = tslib_1.__assign({}, query, {
    ...
    (_a[itemKey] = result[itemKey].map(function(item) {
      ...
      return tslib_1.__assign({}, item, {
        ...
        localId: item.localId + '-' + _this.fakeCursor,
      });
    })),
  ...
  MockTaskDecisionResource.prototype.toggleTask = function(objectKey, state) {
    ...
    return new Promise(function(resolve) {
      ...
      if (cached) {
        ...
        _this.cachedItems.set(
          key,
          tslib_1.__assign({}, objectKey, { state: state }),
        );
      }
      resolve(state);
  ...
  MockTaskDecisionResource.prototype.subscribe = function(objectKey, handler) {
    ...
    debouncedTaskStateQuery = window.setTimeout(function() {
      _this
        .getTaskState(Array.from(_this.batchedKeys.values()))
        .then(function(tasks) {
          tasks.forEach(function(task) {
            var containerAri = task.containerAri,
              objectAri = task.objectAri,
  ...
  MockTaskDecisionResource.prototype.unsubscribe = function(
    objectKey,
    handler,
  ) {
    var key = objectKeyToString(objectKey);
  ...
  MockTaskDecisionResource.prototype.notifyUpdated = function(
    objectKey,
    state,
  ) {
    var key = objectKeyToString(objectKey);
