...
const context = ContextFactory.create({ authProvider: {} as any });
...
containerTypes.forEach(containerType => {
  describe(`Dropzone ${containerType} supplied container`, () => {
    ...
    it('adds "dragover", "dragleave" and "drop" events to container', async () => {
      ...
      component = mount(<DropzoneReact context={context} config={config} />); // Must mount after syping
    ...
    it('should upload files when files are dropped', async () => {
      const component = mount(
        <DropzoneReact context={context} config={config} />,
      );
      const componentInstance = component.instance() as any;
      const spy = jest.spyOn(componentInstance.uploadService, 'addFiles');
