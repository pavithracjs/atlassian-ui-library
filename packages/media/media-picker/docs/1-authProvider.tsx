...
export default md`
  ...
  **AuthProvider** type can be described as:
  ...
  ${code`
  function(mediaClient?: AuthMediaClient): Promise<Auth>`}
 ...
 `}
  ...
  MediaPicker requests access to the signed JWT via the authProvider callback function passed to it as part of ModuleConfig. Auth provider is a function which takes an authMediaClient, and returns a promise of an Auth.
  ...
  export interface AuthMediaClient {
    readonly collectionName?: string;
  }

  export type AuthProvider = (mediaClient?: AuthMediaClient) => Promise<Auth>;
 `}

  The function takes 1 argument:

  1.  _mediaClient_ - object with additional information for auth generation.
      MediaClient is optional:
  ...
  ${code`
  function authProvider(mediaClient) {
    return fetch(https://get-auth?collection=mediaClient.collectionName);
  }
 `}
 `;
