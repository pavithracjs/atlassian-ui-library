...
describe('MediaPickerComponents', () => {
  ...
  beforeEach(() => {
    ...
    pluginState.hasUserAuthProvider = jest.fn();
  ...
  describe('<BrowserWrapper />', () => {
    it('should not render <BrowserWrapper /> component if mediaState.hasUserAuthProvider() is true', () => {
      pluginState.hasUserAuthProvider = jest.fn().mockReturnValue(true);
      wrapper = mount(<MediaPickerComponents mediaState={pluginState} />);
      expect(wrapper.find(BrowserWrapper)).toHaveLength(0);
      expect(wrapper.find(BrowserComponent)).toHaveLength(0);
    });

    it('should render <BrowserWrapper /> component if mediaState.hasUserAuthProvider() is false', () => {
      pluginState.hasUserAuthProvider = jest.fn().mockReturnValue(false);
      wrapper = mount(<MediaPickerComponents mediaState={pluginState} />);
